...
class ClockView @JvmOverloads constructor(
    ...
    private var NumberRadius = 500.0f // 数字显示的位置半径
    ...
    private var HourseColor = 0

    private  var digitToCenterX = -65.0f
    private  var digitToCenterY = 200.0f
    ...
    init {
        ...
        HoursPaint.color = DEFAULT_SECONDARY_COLOR
        HoursPaint.textSize = 40.0f
    }
    ...
    private fun drawHoursValues(canvas: Canvas) {
        ...
        // - hoursValuesColor
        for (i in 1..12){
            val degree = i * 5.0f * UNIT_DEGREE
            canvas.drawText((i).toString(),
                centerX.toFloat() + NumberRadius * sin(degree),
                centerY.toFloat() - NumberRadius * cos(degree),
                HoursPaint )
        }
    }
    ...
    private fun drawNeedles(canvas: Canvas) {
        ...
        // 画分针
        // todo 2: 画分针
        //因为drawPointer中value类型为Int，所以秒引起的分针偏移不做处理
        ...
        // 画数字时间
        canvas.drawText("$nowHours:$nowMinutes:$nowSeconds",
            centerX.toFloat() + digitToCenterX, centerY.toFloat() + digitToCenterY, HoursPaint)
    }
